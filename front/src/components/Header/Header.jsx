import React, {useRef, useState} from 'react';
import Bosom from '../../assets/images/logo_bosom_named.png';
import styles from './Header.module.scss';
import {Link, NavLink} from 'react-router-dom';
import Search from '../../assets/images/search.png';

const Header = ({authed}) => {
    const [isSearchVisible, setSearchVisible] = useState(false);
    const [expanded, setExpanded] = useState(false);
    const nav = useRef();

    const getSearchVisible = () => {
        setSearchVisible((prev) => !prev);
    };
    const getBtnCross = () => {
        setExpanded((prev) => !prev);
    };
    const hideMenu = () => {
        nav.current.classList.remove('show');
        setExpanded(false);
    }


    return (
        <header className={`${styles.header}`} id="header">
            <nav className="navbar navbar-expand-lg h-100 p-0 navbar-custom">
                <div className="container">
                    <Link to="/" className={`navbar-brand ${styles.logo}`}>
                        <img src={Bosom} alt="Bosom"/>
                    </Link>
                    <button
                        className="navbar-toggler"
                        type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#navbarSupportedContent"
                        aria-controls="navbarSupportedContent"
                        aria-expanded="false"
                        aria-label="Toggle navigation"
                        onClick={getBtnCross}
                    >
                        <span className={!expanded ? "navbar-toggler-icon" : "btn-close"}></span>
                    </button>
                    <div
                        className="collapse navbar-collapse d-lg-flex justify-content-lg-between"
                        id="navbarSupportedContent" ref={nav}
                    >
                        <ul className="navbar-nav me-auto ms-lg-2 mb-2 mb-lg-0 w-100 justify-content-center">
                            <li className="nav-item">
                                <Link to="/" className="menu_link" aria-current="page" onClick={hideMenu}>
                                    Главная
                                </Link>
                            </li>
                            <li className="nav-item dropdown">
                                <div
                                    className="menu_link dropdown-toggle"
                                    role="button"
                                    data-bs-toggle="dropdown"
                                    aria-expanded="false"
                                >
                                    Общество
                                </div>
                                <ul className="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Устав
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Структура
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Нормативные документы
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Компании-партнеры
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/signup" className="dropdown-item" onClick={hideMenu}>
                                            Вступить в общество
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Оплатить взносы
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Фотогалерея
                                        </Link>
                                    </li>
                                </ul>
                            </li>
                            <li className="nav-item dropdown">
                                <div
                                    className="menu_link dropdown-toggle"
                                    role="button"
                                    data-bs-toggle="dropdown"
                                    aria-expanded="false"
                                >
                                    Обучение
                                </div>
                                <ul className="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Центры
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/" className="dropdown-item" onClick={hideMenu}>
                                            Курсы
                                        </Link>
                                    </li>
                                    <li>
                                        <Link to="/videos" className="dropdown-item" onClick={hideMenu}>
                                            Учебные материалы
                                        </Link>
                                    </li>
                                </ul>
                            </li>
                            <li className="nav-item">
                                <Link to="/news" className="menu_link" aria-current="page" onClick={hideMenu}>
                                    Новости
                                </Link>
                            </li>

                            <li className="nav-item">
                                <Link to="/login" className="menu_link" aria-current="page" onClick={hideMenu}>
                                    Контакты
                                </Link>
                            </li>

                            <li>
                                <button
                                    className={`btn ${styles.headerSearchBtnHandler}`}
                                    onClick={getSearchVisible}
                                    type="button"
                                >
                                    <svg
                                        version="1.1"
                                        width="28px"
                                        height="28px"
                                        viewBox="0 0 122.879 119.799"
                                    >
                                        <g>
                                            <path
                                                d="M49.988,0h0.016v0.007C63.803,0.011,76.298,5.608,85.34,14.652c9.027,9.031,14.619,21.515,14.628,35.303h0.007v0.033v0.04 h-0.007c-0.005,5.557-0.917,10.905-2.594,15.892c-0.281,0.837-0.575,1.641-0.877,2.409v0.007c-1.446,3.66-3.315,7.12-5.547,10.307 l29.082,26.139l0.018,0.016l0.157,0.146l0.011,0.011c1.642,1.563,2.536,3.656,2.649,5.78c0.11,2.1-0.543,4.248-1.979,5.971 l-0.011,0.016l-0.175,0.203l-0.035,0.035l-0.146,0.16l-0.016,0.021c-1.565,1.642-3.654,2.534-5.78,2.646 c-2.097,0.111-4.247-0.54-5.971-1.978l-0.015-0.011l-0.204-0.175l-0.029-0.024L78.761,90.865c-0.88,0.62-1.778,1.209-2.687,1.765 c-1.233,0.755-2.51,1.466-3.813,2.115c-6.699,3.342-14.269,5.222-22.272,5.222v0.007h-0.016v-0.007 c-13.799-0.004-26.296-5.601-35.338-14.645C5.605,76.291,0.016,63.805,0.007,50.021H0v-0.033v-0.016h0.007 c0.004-13.799,5.601-26.296,14.645-35.338C23.683,5.608,36.167,0.016,49.955,0.007V0H49.988L49.988,0z M50.004,11.21v0.007h-0.016 h-0.033V11.21c-10.686,0.007-20.372,4.35-27.384,11.359C15.56,29.578,11.213,39.274,11.21,49.973h0.007v0.016v0.033H11.21 c0.007,10.686,4.347,20.367,11.359,27.381c7.009,7.012,16.705,11.359,27.403,11.361v-0.007h0.016h0.033v0.007 c10.686-0.007,20.368-4.348,27.382-11.359c7.011-7.009,11.358-16.702,11.36-27.4h-0.006v-0.016v-0.033h0.006 c-0.006-10.686-4.35-20.372-11.358-27.384C70.396,15.56,60.703,11.213,50.004,11.21L50.004,11.21z"/>
                                        </g>
                                    </svg>
                                </button>
                            </li>
                        </ul>

                        <div className={`nav-item mb-2 ms-auto ${styles.headerAccountIcon}`}>
                            <Link to="/login" className="nav-link" aria-current="page" onClick={hideMenu}>
                                {authed ? (
                                    <svg
                                        height="32px"
                                        width="32px"
                                        version="1.1"
                                        viewBox="0 0 60.671 60.671"
                                    >
                                        <g>
                                            <ellipse
                                                cx="30.336"
                                                cy="12.097"
                                                rx="11.997"
                                                ry="12.097"
                                            />
                                            <path
                                                d="M35.64,30.079H25.031c-7.021,0-12.714,5.739-12.714,12.821v17.771h36.037V42.9 C48.354,35.818,42.661,30.079,35.64,30.079z"/>
                                        </g>
                                    </svg>
                                ) : (
                                    <svg
                                        height="32px"
                                        width="32px"
                                        viewBox="0 0 1024 1024"
                                        version="1.1"
                                    >
                                        <path
                                            d="M170.666667 512a42.666667 42.666667 0 0 0 42.666666 42.666667h323.84l-98.133333 97.706666a42.666667 42.666667 0 0 0 0 60.586667 42.666667 42.666667 0 0 0 60.586667 0l170.666666-170.666667a42.666667 42.666667 0 0 0 8.96-14.08 42.666667 42.666667 0 0 0 0-32.426666 42.666667 42.666667 0 0 0-8.96-14.08l-170.666666-170.666667a42.666667 42.666667 0 1 0-60.586667 60.586667l98.133333 97.706666H213.333333a42.666667 42.666667 0 0 0-42.666666 42.666667zM725.333333 85.333333H298.666667a128 128 0 0 0-128 128v128a42.666667 42.666667 0 0 0 85.333333 0V213.333333a42.666667 42.666667 0 0 1 42.666667-42.666666h426.666666a42.666667 42.666667 0 0 1 42.666667 42.666666v597.333334a42.666667 42.666667 0 0 1-42.666667 42.666666H298.666667a42.666667 42.666667 0 0 1-42.666667-42.666666v-128a42.666667 42.666667 0 0 0-85.333333 0v128a128 128 0 0 0 128 128h426.666666a128 128 0 0 0 128-128V213.333333a128 128 0 0 0-128-128z"/>
                                    </svg>
                                )}
                            </Link>
                        </div>
                    </div>
                </div>
                <form
                    className={isSearchVisible ? 'd-flex ms-2 mb-2' : 'd-none'}
                    role="search"
                >
                    <input
                        className={`me-2 ${styles.headerSearchInput}`}
                        type="search"
                        placeholder="Поиск"
                        aria-label="Search"
                    />
                    <button className={`btn ${styles.headerSearchBtn}`} type="submit">
                        <img src={Search} alt="search"/>
                    </button>
                </form>
            </nav>
        </header>
    );
};

export default Header;
